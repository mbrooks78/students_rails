class StudentsController < ApplicationController
  before_action :set_student, only: [:show, :edit, :courses, :course_add, :course_remove, :update, :destroy]
  ...
  def course_add
    ...
    if not @student.enrolled_in?(@course)
      #add course to list using << @course
  ...
  # POST /students/1/course_remove?course[]=
  def course_remove


    #get list list of courses to remove from query string
    @student = Student.find(params[:id])
